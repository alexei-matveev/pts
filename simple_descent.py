...
def chain_for_fun(raw, fun):
    """
    Generates  the chain  corresponding to  raw which  holds  the same
    coordinates but in internal coordinates so that raw = fun(result)
    """
...
def chain_to_points(chain, fun):
    """
    Chain in internal coordinates to chain in cartesian coordinates
    """
...
def relax_points(fun, points, tolerance):
    ...
    feeding them  into find_connections. It  is tried to  converge the
    points to tolerance * 0.1 as it is supposed that tolerance is used
    for the find_connections  also and then it might  be favourable if
    the points are better than the  trial for the next step. But it is
    only  checked if  tolerance is  reached,  as this  is the  minimal
    requirement lateron. Some warnings are given back if the minimizer
    returned with  something strange but  only the gradients  too high
    ...
    for i, point in enumerate(points):
         ...
         # See docstring of mueller_brown module for the strategies we
         # expect  something  with  MIN1,  TS1, MIN2,  TS2,  MIN3,  ..
         ...
         if i % 2 == 0:
             ...
             def g2(x):
                  ...
                  transititon states,  but here  w do not  really have
                  the derivatives (or want to have them
                  """
                  g = fun.fprime(x)
                  return dot(g, g)
...
def steepest_descent(force, x0, fixedlength = True, alpha = 0.001):
    """
    Simple steepest  descent step, forces  should be belonging  to the
    paramter x  x0 is the old  place fixedlength = True  will norm the
    step  of the  object alhpa  is the  scaling factor  for  the step,
    together with fixedlenght = True it is the steplength This version
    does not consider metric
    """
    step = force
    if fixedlength:
...
def steepest_descent_met(metric, force, x0, fixedlength = True, alpha = 0.01):
    """
    Simple steepest descent step,  function fun should give the forces
    belonging to the paramter x x0 is the old place fixedlength = True
    will norm the  step of the object alhpa is  the scaling factor for
    the step,  together with fixedlenght  = True it is  the steplength
...
def steepest_descent_path_simple(fun, x0, metric, store_every = 1,
    ...
    """
    Steepest descent path, with simple steepest descent step.
    """
    path = [x0]
    ...
    for it in range(max_iter):
        ...
        # Loop will break  here or if there are  too many steps, don't
        # want to try for infinity
            break
        if it % store_every == 0:
            path.append(xval)
...
def find_connections(fun, points, metric = Default(None), **params):
    ...
    and  transition  points).  points  should contain  all  stationary
    points the path will cross
    ...
    for i in range(len(ts)):
         ...
         # We  want the  path min1-ts-min2,  but  there is  no way  of
         # telling  if   direction  =  True  is   min1-ts  or  ts-min2
         # beforehand, thus now  we have to decide if  the second path
         # piece comes before or after the first one
         if min == 1:
...
def path_l_to_r(metric, fun, start, reactant, product, direction, simple = False, tol_points = 1e-1, **params):
    """
    Generates a  path going in steepest descent  direction from start.
    As  the  reaction path  is  supposed  to  go from  reactants  over
    transition  state to  products the  path piece  start-reactants is
    switched.  The  convergence obtained by the  steepest descent path
    is checked.   Path is forwared  together with the result  on which
    path  piece could be  deteceted in  the returned  path.  direction
    decides  if the  starting vector  is negated,  allowing  easily to
    check both paths.

    This version considers metric
    """
    # the actual steepest descent path calculation
    if simple:
    ...
    # Ckeck  if  the results  from  the  path  are somewhat  near  the
    ...
    if dot(diff1, diff1_down) < tol_points  and dot(diff1, diff1_down) < dot(diff2, diff2_down):
        # We   got   as    path:   start-reactants,   switch   it   to
        # reactants-start
