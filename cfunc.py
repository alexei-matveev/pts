...
CoordSys2.  They  are all  with fprime /taylor  attached as  funcs. Be
aware that they  expeced when used as wrappers, that  they get also an
inner function which is a Func.
...
Ensure  that the derivatives  are correct:  Consider that  NumDiff has
size (n, 3, m), while we get normally something of size (n*3, m) back.
...
Now have some values repeated: Both bond-length are the same:
...
class With_globals(Func):
    """
    Adds to  function fun_raw some global parameter  for: rotation (as
    vector  description  of quaternion)  translation  of the  complete
    molecule.

    The rotation and translation is  calculated with rt object.  It is
    supposed that for each  internal coordinates given, the last three
    coordinates are  the global translation and the  three before that
    are for the global rotation.

    After each  calculation of  the Cartesian coordinates  by fun_raw,
    they are adapted with the global parameter.
    """
    ...
    def taylor(self, y):
        ...
        dx.shape = (-1, 3, len(y)) # ( number of atoms, 3, number
...
class Mergefuncs(Func):
    ...
    functions.  For initalization it also needs to know the dimensions
    of the needed internal coordinate arrays for the single functions,
    ...
    def taylor(self, x):
        ...
        # consider here which part of the complete internal coordinate
        # vector belongs to it (stored in dims)
        ...
        assert (iter == len(x))
        # the  Cartesian coordinates have  only to  be brought  to the
        # correct shape
        res = hstack(res) # add all the coordinates up
        res.shape = (-1, 3) # (number atoms, 3)

        # for  the  derivatives some  more  has  to  be done:  as  the
        # functions does  not interact which each other,  one has fill
        # up the derivatives with  zeros the single derivative results
        # will be put on the diagonals of the complete matrix
        ...
        for  m1, dim in zip(dres, self._dims):
             ...
             i += a
             j += dim
        mat.shape = (-1, 3, iter) # number atoms, 3, number internal
                                  # coordinates
...
class Masked(Func):
    ...
    def taylor(self, x):
        ...
        # transpose mat back
        mat = mat.T
        mat.shape = (-1, 3, len(x)) # number atoms, 3, number internal
                                    # coordinates
...
class With_equals(Func):
    """
    Is a  wrapper function around  fun_raw which considers  that there
    are  several variables  with the  same value  (as given  by mask),
    which have  to be  given several times  to the inner  function The
    mask is here a list of integers. It gives for each wanted internal
    coordinate  of the  inner  function which  value  of the  internal
    coordinates given to with_equals should be used (it is supposed to
    be several times the same one)
    """
