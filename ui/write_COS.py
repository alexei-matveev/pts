...
def print_xyz_with_direction(write_method, coord, cs, text = "", direction = None):
    """
    Prints the geometry coord (after transformed in Cartesian) and the
    direction into the write_method.

    INPUT:
    write_method: should be a write method, like fopen(filename, "w").write or
                  sys.stdout.write.
