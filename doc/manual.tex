...
\geometry{verbose,tmargin=1.5cm,bmargin=1.5cm,lmargin=2.5cm,rmargin=2.5cm,headheight=1.5cm,headsep=1.5cm,footskip=1.5cm}
...
Thus the required path can just be: /users/alexei/PYTHON-TESTING (for
actual ParaTools version). So for a bash shell one has to do:

\textit{export PYTHONPATH=/users/alexei/PYTHON-TESTING:\$\{PYTHONPATH\}}

or for a tcsh shell:

\textit{setenv PYTHONPATH /users/alexei/PYTHON-TESTING:\$\{PYTHONPATH\}}
...
\textit{/users/alexei/PYTHON-TESTING/pts/bin/}

Now ParaTools is ready for general usage. However, be aware that most
of the quantum chemistry calculators need also some more specific
environment variables to make them run correctly.

For example, this would be for VASP: 
...
(the last one is only an example, that will make all actual VASP calculations
run on separate processes scheduled from the python program on its
own with two processors each. This parameter is required to be adapted
to how it should run. See Section \ref{sub:Parallel-local-cluster}.)

\smallskip{}


All the functionalities of ParaTools are collected in the central
command. This executable is situated in the directory\textit{ pts/bin/},
which has to be present in \textit{\$PYTHONPATH ,} and is called paratools.
The usage is the following:

\textit{paratools CMD {[}}\nobreakdash-\nobreakdash-\textit{help|
...{]}}

CMD is the specification for the choice of paratools functionalities.
Thus it can be for example string, forces or frequencies. For each
of the CMD the option --help provides a description of how the special
command works and which options can or have to be set. Some of these
commands share the same interface, thus some of them (like the different
pathsearcher functions) will provide the same help text. To see a
list of all available commands for paratools call
...
To run a calculation searching for the minimum energy path do:
...
to interpret the output of paratools pathsearcher routines or visualize
parts of them. They might require the output of a preceeding neb or
string calculation. But others are independent of it or allow another
kind of usage. 

Another example is the tool ts\_and\_mods, for which the simplest
kind of usage looks like: 
...
Usually one might omit some of the steps, for example if the minimum
is already completely relaxed. This example is more designed to show
...
tools might be useful, if not in all calculations then at least sometimes.
...
start: 
\begin{itemize}
\item The required setting for the initialization are about changing some
...
\textit{export PATH=/users/alexei/PYTHON-TESTING/pts/bin/:\$\{PATH\}}
...
\textit{setenv PATH /users/alexei/PYTHON-TESTING/pts/bin/:\$\{PATH\}}
...
VASP POSCARs can also contain information about the fixing of some
(Cartesian) coordinates, if selective dynamics is set. ParaTools can
also interpret and use this, at least as long as all the calculations
...
which are normally specified in a separate file if only VASP is used.
...
{}``cells'' without changing these results. ParaTools however does
...
It will not think that there might be infinitely duplicates at this
position plus a natural multiplicity of the cell vectors. Thus it
is highly recommended to test the initial path of the pathsearcher.
The tool make\_path (chapter \ref{sub:Makepath.py}) will build an
interpolated path through the coordinates it gets. As it calculates
only the geometries of the path it is rather quick. Given the same
input as pathsearcher it will build up the start path of pathsearcher.
As output it gives (in xyz format) the geometries of all the beads.
...
situated that they have to move only the shortest path between them.
...
But the last path may not be the best one as there can appear fluctuations
...
\begin{itemize}
\item On default all information of interest are stored in the path.pickle
...
It would be also possible to extract most of the information from
...
direct inspection. This can be done by one the visualizing tools (chapter
\ref{sub:Path2xyz.py}).


\textit{paratools xyz path string.debug30.path.pickle \nobreakdash-\nobreakdash-beads }
...
The option \nobreakdash-\nobreakdash-modes makes ts\_and\_mods further
...
with the tool {}``xyz path''.}\label{fig:Path-30}}
...
other directions should be minimized. The difference between dimer
and lanczos is the way said special direction is updated from step
to step. To start such a calculation one needs an initial geometry,
here POSCAR.ts as result from ts-and-mods copied into a POSCAR for
having all the additional information from it ready. Again the calculator
...
beads if not explicitly asked for an interpolated path but in all
other things it should follow the same interface as {}``plot path''.
...
has been generated with the tool {}``plot path'' (see chapter \ref{sub:path2plot.py}).}
...
ParaTools provides also a wrapper to this methods in its central command
...
system (ASE will try to reuse the additional information, as there
...
might be different from the one in which the calculator is created
...
occupations set), which will somehow join their results later on.
Normally ParaGauss would write the input file anew each time it starts
a calculation (copy\_input = {}``always''). Alternatively one can
forbid him to write input files (copy\_input = {}``never'') or allow
only to write them if they are not already there (copy\_input = {}``inexistent'').
...
subdirectory (as already mentioned) it might be convenient to ask
...
with each other. The list can be specified directly. For the example
...
while the other contains all the atoms of the adsorbate.}\label{fig:DFT+D} }
...
by giving an additional interaction matrix. This matrix tells if there
...
{}``plot path'', chapter \ref{sub:path2plot.py} )\label{fig:Some-internal-coordinates}}}
...
wrapper around VASPs POSCAR) might also contain some information about
fixing of coordinates there is another way of getting this information.
...
Thus a full line holds the information: number of atom 1 (the new
...
category (distances, angles, etc.) or if they start with a similar
...
Alternatively the zmatrix information can also be extracted from gxfiles.
It is not intended as a general strategy for users to create a zmatrix
as a gxfile, but ParaGauss users might have already one for their
systems, which then can be reused. Here it does not suffice to give
the gxfiles as geometry files but the gxfile has to be given also
explicitly with the command \textit{\nobreakdash-\nobreakdash-zmatrix
<name of gxfile>}. Additionally one has to set\textit{ \nobreakdash-\nobreakdash-zmt\_format
...
if no other information are provided about that. Be aware that the
...
\item format: ParaTools (through ASE) know several formats for geometry
...
it will recognize the POSCAR in it and interpret it as VASP POSCAR
...
information in the atoms object, the object which stores the geometry
information of the minima. It could be set there directly, but it
is more common to expect it to comes from the input file. In POSCARs
for example one may use {}``selective dynamics'', which fixes some
of the coordinates. This information can be interpreted from ASE as
constraints, which then can be used from ParaTools to fix some coordinates.
For general usage with the constraints (via python code): ParaTools
can interpret the {}``FixScaled'' and the {}``FixAtoms'' constraints
of ASE. ParaTools only fixes coordinates this way if they are the
only mask available and if only Cartesian coordinates are used.
...
by several of paratools other tools and allow to quicker get information
...
codes will use as initial path an linear interpolation in all the
variables of the two minima. But this path may be totally off. First
in calculations with periodic boundary conditions one has to ensure
that none of the atoms is in different cells in the two geometries.
But this may not be sufficient. Linear interpolated atoms might show
completely unchemical behavior as tunneling through a surface instead
of moving around its edge. This might make it unusual hard or even
impossible for the path-searcher routines to converge towards the
real path.

In this cases there is the chance of giving some additional geometries,
which should be on a better initial path. 

In internal coordinates there might appear another problem: Not all
internal variables are uniquely defined by the Cartesian geometries.
Dihedral angles and the quaternions describing the global rotation
...
the longer one (with the norm of dihedral angles larger than $\pi$
between the minima) are wanted there is need for giving some more
information on the initial path. Note that the reduction on angles
smaller than $\pi$ is only done between succeeding geometries, thus
the complete path may go over bigger angles. This reduction is only
...
the other direction than has been chosen as initial path. 
...
current chapter. The standard output is more or less debug information,
the only interesting part of it is, if it announces at the end {}``Optimization
...
which can be Z-Matrix coordinates, flattened Cartesian coordinates
...
information about the bead, which were put apart from the first sections.
...
about the content already. There is {}``internal\_coordinates'',
...
Angstroms or radians). Another one, {}``abscissa'', contains their
positions on the path for path parametrization, thus the first value
should there always be 0 and the last one 1. As the beads needn't
be equal spaced in the path variable, not even (completely) for a
...
{}``output\_geo\_format'' to the ASE shortcut for the wanted format. 
...
The string.path.pickle file contains all information of the last path
in an ParaTools readable format (not human readable through). Thus
it can not be used directly, but the toolbox provides a lot of other
nice tools, which uses this as an input. So for example the other
transition state estimate methods can be extracted from it. The tools
should accept also other formats of the input, like the files in \ref{sub:Files-after-finished},
but then in general more than one file is needed and it has to make
clear to the method which file is which. The path.pickle file is an
elegant way to move the information in a simple form. On the other
hand path.pickle files are more sensible. They are not longer readable
any more if the format changes or the python code changes its pickling
methods too much. Thus they are more something for the direct following
observations, not for long time savings of the results.
...
using the same interface. Except when differences are explicitly mentioned
as in \ref{sub:Differences-between-Dimer/Lanczos} the explanations
are holding for both methods, even when they are only mentioning Dimer.


\subsection{The required input for a Dimer/Lanczos method\label{sub:Input-Dimer-lanczos}}
...
parameters to interpret it are interpreted exactly the same way as
it described for the pathsearcher routine in Sections \ref{sub:Geometry_path},
...
has to be given as above but all other information like the geometry,
the mode vector and the coordinate system (internal, mixed, Cartesian)
can be extracted from a path.pickle file. This file contains information
...
is chosen as tangent of the path at the transition state estimate.
...
it is highly recommended to have a look at them and their meaningfulness
...
the convergence criteria are met (Implementation details prefer to
use one such image and the midpoint, only approximating the second
image). The Lanczos method is a direct method for eigenvalue determining.
...
Especially with the more rougher convergence criteria they should
perform very similar. However if a very exact result is needed Lanczos
usually outperforms Dimer. Lanczos needs at least the number of degrees
of freedom, as in this case the subspace would collapse to the complete
...
setting variables, directly in the command line or specified in a
...
However there are some more parameters to steer the dimer method with.
...
Parameter which are explicitly free for adaption are:
\begin{itemize}
...
are called and summed up and used as abortion criteria. Be aware that
...
as an angle in radians: for Dimer the angle the system would rotate
...
\item logfile <file>: Setting this parameter to a valid value (other than
{}``None'' or {}``-'') will redirect the statistics of the standard
output to <file>. The result of the calculation (the last geometries
and so on) will still be directed to standard output.
\item trajectory <level>: While the calculation proceeds the geometry and
the modevector will be changed continuously. This parameter is about
how much of this can be found later on in the subfolder the calculation
runs in. All geometry files given for the trajectory are in xyz-format,
the modes contain a matrix of floats. All the files are human readable.
There are different levels <level>, which have to be given as a string:
...
The step information consists of three lines, which are repeated for
...
lines in the start input. They give the information named there. Thus
about the translation we have the information: 
...
The length of the step is followed by the length of the step components
perpendicular and parallel to the current mode vector. Angle to mode
describes the angle between step and mode vector. The max force component
...
same can be done for the rotation information:
...
This section gives all information which the dimer method has stored
...
help to recover additionally needed information. 
...
style independent of the internal coordinates used.


\section{A simple Quasi-Newton method}

A minimal usage would look something like:

\textit{paratools quasi-newton \nobreakdash-\nobreakdash-calculator
<calculator file> <geometry file> }

The quasi-Newton method is used as a simple transition state (or minimum)
search. It follows the same principal interface as the dimer and lanczos
methods, see Section \ref{sub:Input-Dimer-lanczos}, with the main
difference that it does not require (and does not accept) a mode vector
file. Thus it is enough to refer to Section \ref{sub:Input-Dimer-lanczos}
for the general method description and explain only the parameter
which can be chosen for the quasi-Newton method. The defaults are
again available by
...
The parameter are:
\begin{itemize}
\item max\_iteration <N>: The method performs <N> iterations at most.
\item converged <threshold>: Set the threshold for the gradient convergence
criteria, the maximal absolute value of the gradient, to <threshold>.
\item update\_method <method>: Selects the method <method> for the Hessian
update of the quasi-Newton method. The choice {}``SR1'' is fit for
transition state search (in fact it searches for the nearest stationary
point) while the other choices , {}``BFGS'' and {}``LBFGS'', can
only be applied reasonably for a minimum search.
\item max\_step <step\_size>: Sets maximal step length for the step to <step\_size>.
\item logfile <file>: Setting this parameter to a valid value (other than
{}``None'' or {}``-'') will redirect the statistics of the standard
output to <file>. The result of the calculation (the last geometries
and so on) will still be directed to standard output.
\item trajectory <level>: While the calculation proceed additional input,
like the geometry and gradients can be stored in some files. See Section
\ref{sub:parameter_Dimer} for valid values of the string <level>.
Note only, that as quasi-Newton methods do not use any mode vectors
the corresponding output files will not be created.
\end{itemize}

\section{Visualization Tools and Tools for converting pickle files\label{sub:path2plot.py}}

These tools are applied via the commands plot, show, table and xyz
of paratools. Each of these commands will give a special output format.
Plot and show will generate a plot of arbitrary internal coordinates
(compare figure \ref{fig:path2plot}). The difference here is whether
they store them directly as a picture (plot) or give them as a pop-up
window (show). Table is connected with them but will give the requested
values in a table. The xyz tools will give a string of xyz files,
extracted from the current pickle file. 

The next argument is the input format, as there are several choices.
It can be path, then the input files have to be path.pickle files
or it can be xyz, which requests files containing xyz-geometries.
Of course the combination output xyz and input xyz does not make any
sense and will not work.

Usage of it:

\textit{paratools show path \nobreakdash-\nobreakdash-dis 1 2 string.path.pickle
\nobreakdash-\nobreakdash-dis 1 3 }

\textit{paratools table path \nobreakdash-\nobreakdash-dis 1 2 string.path.pickle
\nobreakdash-\nobreakdash-dis 1 3 }

Additionally also progress files of the dimer/lanczos method can be
used as input format, given as show progress, for example. 

%
\begin{figure}[H]
\includegraphics[scale=0.8]{Pictures/Ethanoxiddev_ito30}

\caption{\textbf{The command plot is designed to show selected internal coordinates
(or variables from the .log file) in a graphical way. This is done
here for a {}``path'' as input. It allows to see how the path in
the given variables looks like. Watch out for too steep drops, loops
and such kinds to find out how good your given path is. The picture
here shows two internal coordinates from the example given in chapter
\ref{sec:How-to-Find} for three iterations. The number 1 after the
name of them is there if several values, like several angles are looked
at the same time.\label{fig:path2plot}}}

\end{figure}



\subsection{The general interface\label{sub:The-general-interface}}

There are two different ways of putting the main input into the routine:
the path.picke file contains all the information required but as in
tool ts-and-mods in \ref{sub:Tsestandmods.py} it is also possible
...
of a single image and all but the masked values might be nonsense
(0.0 for example). They are provided as \textit{\nobreakdash-\nobreakdash-mask
<maskfile> <geometry file>} to the tool. If available also abscissa
...
\hline 
kind & abbreviation & num atoms & specialty\tabularnewline
\hline
\hline 
distance & dis & 2 & distance a-b\tabularnewline
\hline 
angle & ang & 3 & angle a-b-c\tabularnewline
\hline 
angle4 & ang4 & 4 & angle between a-b and c-d\tabularnewline
\hline 
dihedral & dih & 4 & dihedral of a-b-c-d\tabularnewline
\hline 
plane-distance & dp & 4 & distance of a to plane by b,c and d\tabularnewline
\hline 
line-distance & dl & 3 & distance of a to line of b $\rightarrow$c\tabularnewline
\hline 
on-plane & on & 4 & coordinates of projected a on plane\tabularnewline
 &  &  & given by b and b-c and b-d\tabularnewline
\hline
\end{tabular}
...
only available for inputs of path.pickle or progress.pickle files
as reading in extra files for energies and forces only to give the
values back nearly unchanged does not make much sense. For the energies
all that can be done is given them back, while of the gradients some
different values can be extracted. The names for the options are the
following:
\begin{itemize}
\item \nobreakdash-\nobreakdash-energy or \nobreakdash-\nobreakdash-en
will process the energies
\item \nobreakdash-\nobreakdash-energy2 will process the energies (paths
are interpolated with usage of gradients)
...
\item \nobreakdash-\nobreakdash-ts-estimates <num>: The transition state
after method <num>, compare \ref{sub:Tsestandmods.py}, is also calculated
and the relevant data is extracted. For the output formats show and
plot it is added to the figure, for the output format table it is
replaced instead of the other information. Remember that for most
transition state estimates energies and gradients are only extrapolated.
...
is need of two additional information for the system: 
\begin{itemize}
...
much to shift in which direction. These values are stored in the cellfile.
It has to be a file and should contain the unit cell in the following
format:
...
The shifted atom will have the number $N+s_{n}$, with $N$ being
the number of atoms in the original description and $s_{n}$ being
the number of the line of the atom in expandlist. 
...
\subsection{The accepted input formats}


\subsubsection{The input format {}``path''\label{sub:The-input-format}}

Simplest usage with path.pickle files:

\textit{paratools {[}output{]} path string.path.pickle {[}options{]}}

However, there are two different ways of putting the main input into
the routine: next to the path.picke file, which contains all the information
required, it is possible, as in tool ts-and-mods in \ref{sub:Tsestandmods.py},
to give it in internal geometry files. They can be of the kind the
path\_searcher creates as internal\_geometries. Then it is required
to provide the symbols in a separate symbolfile, in which they are
separated by whitespace. This file is given once as \textit{\nobreakdash-\nobreakdash-symbols
...
\subsubsection{The input format {}``xyz''}

The input files contain strings of xyz geometries which are treated
as if they were a progress string or a path file. However it is not
possible to get energy or forces with this input format. Thus it is
also not possible to extrapolate a transition state from them. On
the other hand all the geometry information can be extracted.


\subsubsection{The input format {}``progress''\label{sub:The-input-progress}}

This input format uses the pickle files from a dimer/lanczos or quasi-newton
calculation. They contain the geometries, energies and gradients from
the dimer/lanczos midpoint or the quasi-newton image during the respective
calculation.

The following parameter are only available for this input (as they
would not make much sense for the other input formats anyway). They
are only related to the values which are plotted or printed:
\begin{itemize}
\item \nobreakdash-\nobreakdash-step: the steplength between succeeding
geometries is displayed 
\item \nobreakdash-\nobreakdash-curvature : provides the curvature along
the mode direction (only available for dimer and lanczos calculations).
\item \nobreakdash-\nobreakdash-modes: plots the mode direction in the
given projection (only dimer/lanczos calculations and only for internal
coordinates) additional to every image.
\item \nobreakdash-\nobreakdash-vector-angle <dir 1> <dir 2>: The angle
between the two directions <dir1> and <dir2> are provided. For the
case that both directions are dependent on the iteration the following
rule holds: If <dir1> = <dir2> the angles are between succeeding iterations,
else they are from the same iterations.
\end{itemize}
The available directions for vector-angle are:
\begin{itemize}
\item mode: the mode direction (only available for dimer/lanczos)
\item grad: the gradient in the given iteration
\item step: the step direction, as direction from the last iteration
\item initmode: the first mode provided (only available for dimer/lanczos)
\item finalmode: the final mode of the last iteration (only available for
dimer/lanczos)
\item init2final: the direction from the first to the last geometry 
\item translation: accumulated translation from initial point
\end{itemize}

\subsection{The output formats}


\subsubsection{Only plot/show}

Plot takes the first internal coordinate given as x-Value and plots
all others against it as y-value. Energy and gradient and lines from
the logfile are always given after the internal coordinates. It is
needed to provide at least one internal coordinate for the x-Value.

The following options are only available for these tools. They are
mainly about how the figure should be plotted:
\begin{itemize}
...
in y direction 
\item \nobreakdash-\nobreakdash-xrange <a> <b> : In x-direction the plot
is restricted to the area x with <a> < x < <b> 
\item \nobreakdash-\nobreakdash-yrange <a> <b> : In y-direction the plot
is restricted to the area y with <a> < y < <b> 
\item \nobreakdash-\nobreakdash-logscale <z> : for z = 1,2 or z = x,y:
sets scale of this direction to logarithmic
\item \nobreakdash-\nobreakdash-output <file>: the output will be going
to <file>. Setting this command for the output format {}``show''
changes the output format automatically to {}``plot''. 
\item \nobreakdash-\nobreakdash-reference <geo-file> <energy-file>: The
reference point with geometry taken from geo-file and energy taken
from energy-file is added (where possible) to the plot (mind that
there is no gradient information included). <energy-file> needs only
to have a valid value if the energy should be shown.
\end{itemize}

\subsubsection{Only for table}
...
\textit{<number>} equal spaced images on it. If the option is used
together with the transition state option the transition state(s)
are provided and \textit{\nobreakdash-\nobreakdash-num }is ignored.


\subsection{Special output format xyz\label{sub:Path2xyz.py}}

This special output format cannot make use of the shared interface
from Section \ref{sub:The-general-interface}. However, it uses the
input formats {}``path'', see Section \ref{sub:The-input-format},
and {}``progress'', see Section \ref{sub:The-input-progress}. The
tool gives a string of jmol type xyz-files to the standard output. 

Run for example:

\textit{paratools xyz path string.path.pickle }

The available parameters are dependent on the input format:

For a {}``path'' input: As default the geometries are equally spaced
in the path coordinate, and there are as many as there are beads in
the input.
\begin{itemize}
\item \nobreakdash-\nobreakdash-number-of-images <N>: there are <N> images
on the path.
\item \nobreakdash-\nobreakdash-beads: the exact bead positions of the
path are provided instead of the geometries on the path.
\end{itemize}
In case that the input was a {}``progress'' file:

Adding the parameter \nobreakdash-\nobreakdash-modes will add the
mode vectors after the geometries. For this the mode vector have been
transformed in Cartesian coordinates. Some programs, like Jmol can
interpret these additional information as a direction vector.


\subsection{Additional plotting tools not accessible with the central command}

These tools were created with the old design. They are still accessible,
however the newer tools added to the central command might be the
better choice for the general usage.
...
bead (number 1) is in this case not the best. The tool ts-and-mods
...
by a file containing the geometries in internal coordinates for all
beads (like the internal\_coordinates file of path\_searcher). But
in order to be able to work it needs not only the geometries but also
...
<abscissafile>.} A NEB calculation would not use abscissas at all
and thus not create this file but the string calculations will provide
it for the last path. 

The geometries of the internal\_coordinates are internal ones. The
tool might therefore need to know how to interpret them. As default
...
These information can be provided by \textit{\nobreakdash-\nobreakdash-mask
...
\begin{itemize}
\item \nobreakdash-\nobreakdash-all : Some transition state estimate methods
can find several transition states on a path. As default they will
only give the one highest in energy. If \nobreakdash-\nobreakdash-all
is given, all transition state estimates are given.
\item \nobreakdash-\nobreakdash-cartesian-modes: Gives some modes (tangent
of the path, vector from first to last image, vector between the two
neighbor images) in Cartesian coordinates.
\item \nobreakdash-\nobreakdash-m, \nobreakdash-\nobreakdash-modes:
Prints the tangent of the path in internal coordinates (the coordinates
applied) for every transition state. 
\end{itemize}
...
information about this Bead nonsense. If a bead was reached at least
...
\textit{paratools find-limit-path string.log \textquotedbl{}RMS Perp
Forces\textquotedbl{} 0.5 -n \textquotedbl{}Bead Energies\textquotedbl{}
0.9 -\% \textquotedbl{}RMS Step Size\textquotedbl{} 0.01 -n -a }
...
: Instead of {}``RMS Perp Forces'' the line of interest will be
...
an eigensolver of the scipy module. The specialty of this function
...
as initial path. As for the path\_searcher methods it is the possible
...
all information about it, except of course for valid energy or force
calculations.
...
should run, specifying how exactly the calculation should be done.
...
fixing some geometries. This is not done by omitting this constraints
from the optimization but rather by resetting them after an optimization
...
\part{Setting up the system to run\label{par:Setting-up-the}}
...
its sources only during run-time and does not provide separate executables).
Thus there might be already for other python functionalities something
inside.

It is also recommended to expand the PATH in order to have direct
access to the tools: 
...
\textit{setenv PYTHONPATH$\;$/users/alexei/PYTHON-TESTING }

and in case the old variable should only be extended and not replaced: 

\textit{setenv PATH$\;$/users/alexei/PYTHON-TESTING/ase/tools:\$\{PATH\} }
...
case: \textit{/users/alexei/PYTHON-TESTING} and maybe some more values,
if there has been anything declared as PYTHONPATH before. This can
be done for all the variables.
...
\section{Getting an Own Version of the Toolbox}


\subsection{Getting ASE\label{sub:Getting-ASE}}

ASE (the atomic simulation environment) is an open source product
of the Center of Atomic-scale Material design (CAMd). Theoretically
it would be possible to get the sources right from them. However be
aware that the distinction their code $\rightarrow$ ASE repository
and our code $\rightarrow$ pts is not completely true as we had also
to do some changes to ASE. The additional calculators like ParaGauss
are also located in our version of ASE. Therefore one should distinguish
between our ASE and the original ASE in these sense.

The source of our ASE are managed locally by darcs revision control
system. There is for example the repository used for the calculations
at \textit{/users/alexei/PYTHON-TESTING/ase}. But this is only accessible
within our local network (and here only on quad4 and the nehalem machines).
It has been linked from the directory \textit{/users/alexei/PYTHON/python-ase-local}.

To get these sources execute

\textit{darcs get /users/alexei/PYTHON/python-ase-local}

To update the repository obtained in this fashion execute

\textit{darcs pull /users/alexei/PYTHON/python-ase-local}
...
To use the new repository for calculations set PYTHONPATH to this
ASE version and do not include one of the others (or only after the
new one). Consider that the ase sources are in a subdirectory of the
directory just pulled; this provides absolutely freedom in the choice
of the name of your repository but the requirement to have the link
include this repository name.


\subsection{Getting pts Sources (ParaTools repository)}

The ParaTools sources not directly related to the ASE calculators
are in the pts repository. Here one could also get the version from
the production repository:

\textit{darcs get /users/alexei/PYTHON-TESTING/pts}

Older versions of pts code and a newer developer version might be
also around\textit{. }They can be found in \textit{/users/alexei/PYTHON
}under different names. For ParaTools it is required that the repository
is named pts, as the darcs repository is the same that contains the
code. Alternatively one can provide a link under the name of pts to
a repository of a different name (see for example the link in PYTHON-TESTING)

To use the code one also has to adapt the PYTHONPATH variable accordingly
(compare \ref{sub:Getting-ASE}). However in this case use the path
to the repository or its link.


\section{How to Run on Different Computers}
...
with these calculators, how the calculators itself are used, has been
explained in \ref{sub:The-calculators}.


\subsubsection{Running with VASP}

There are two variables needed in order to run VASP correctly. The
first one will tell the program, where the pseudopotentials for VASP
are stored. As ASE and thus also ParaTools produces the input files
for VASP by their own, they need the potentials stored somewhere (to
create the POTCAR from them). On our local cluster they can be set
by (consider different shells):
...
The second thing the wrapper need to know is about the VASP program
itself. There are two choices for telling the ParaTools about the
actual position of the VASP executable: setting a VASP\_SCRIPT variable
or setting a VASP\_COMMAND:
...
Be aware that this example stands for running all single point calculations
serial (only one VASP calculation at a time) on our local machines.
For parallel usage we would have for example (see \ref{sub:Parallel-local-cluster}):

\textit{export VASP\_COMMAND=\textquotedbl{}qrsh -cwd -pe \textbackslash{}{*}
2 -now no -N v-single mpirun /users/alexei/bin/vasp\textquotedbl{}}

\bigskip{}


Another way to give all required information to vasp is with a VASP\_SCRIPT.
...
\textit{exitcode = os.system('/users/alexei/bin/vasp') }
...
ParaGauss does not need any additional environment variables. All
what is needed is given directly in the calculator settings. But here
the program also has to get to know, where the executable is situated.
This is done by the variable cmdline, which should be set in the calculator
inputs:
...
\subsection{Running on SuperMUC}
...
The second one loads the correct python version, ParaTools and ASE
settings.


\section{How to Run in Different Ways}


\subsection{Running ParaTools Interactively}

All functionalities of ParaTools are provided in a way, which makes
it possible to use them interactively. This means one can directly
...
case a file of a calculation should be further examined. To make the
handling of the commands easier is all of them will give hints about
their usage, when called with \textit{{}``\nobreakdash-\nobreakdash-help}''. 
...
\item minimize: minimizes the energy of a system
...
\item dimer, lanczos, quasi-newton: local transition state searches
\item dimer-rotate, lanczos-rotate: find smallest eigenvalue of approximate
Hessian and its (mode) direction
\item make-path: interpolation of path between two minima
\item jmol: interpolate path and display with jmol
\item find-limit-path: checks first appearance of limits in logfile
\item min-iter: iteration of minimum in logfile
\item plot path, show path, table path, xyz path: takes path.pickle file
and extracts internal/cartesian coordinates
\item plot progress, show progress, table progress, xyz progress: takes
progress.pickle files and extracts internal/cartesian coordinates
\item ts-and-mods: shows transition states and mode vectors from a path
\item pp2ts-err: errors from TS. approximation to TS
\item transform-zmatrix: transforms ZMatrices into ParaTools Z-Matrix format
\item compare\_geos: calculates differences between coordinates of geometries
\end{itemize}
...
in using the ParaTools modules by accessing them with python. One
of these additional functionality is that the geometries can be expressed
in nearly arbitrary coordinate systems as long as they provide a transformation
into Cartesian coordinates and the corresponding derivatives.
...
of the geometries makes sense: the pathsearcher tools for finding
an minimal energy path and the frequency tool, working with numerical
calculated second derivatives. 

The parallel calculation of geometries is done with a parallel mapping
function. The mapping function decides which geometries to process
when and how many of them might be done in parallel. It is easily
exchangeable but the default is a variant which generates a parallel
strategy taking different processor numbers per single point calculation
into account. As it is difficult to give these information to the
calculators - they usually expect to use all processors they find
if called in a batch job - a one to one usage of this strategy is
...
so that there have to be made some adaptions in order to use it. It
is always invoked when running a pathsearcher tool but for frequency
calculations not.


\section{Serial Calculations\label{sec:Serial-Calculations}}
...
The parameter file has to be provided in the command line. Z.b. for
the method string:

\textit{paratools string \nobreakdash-\nobreakdash-calculator <CALC>
\nobreakdash-\nobreakdash-paramfile params.py GEOM1 GEOM2}

Alternatively the value can be provided directly in the command line:

\textit{paratools string \nobreakdash-\nobreakdash-calculator <CALC>
\nobreakdash-\nobreakdash-cpu\_architecture {[}1{]} GEOM1 GEOM2}

The value for the parameter pmin must not have a bigger number than
the value for the cpu\_architecture.

Just make sure that pmin is not set to a bigger value than the cpu\_architecture
(as default it should be also 1).
...
\subsection{Separately Scheduling of Tasks (Our Local Nehalem Cluster)\label{sub:Parallel-local-cluster}}

The following strategy can be effectively used for scheduling queues
in which small scheduled jobs start quickly after access and several
of them can run per user. One cluster on which this strategy works
is our local Nehalem cluster. First the changes to the calls of the
quantum chemistry programs are described (against the way they are
called serial or for other parallelization strategies) afterwards
how the programs have to be called.

The strategy is described for the scheduling program {}``sun grid
engine'', which runs on our local cluster and might be adapted for
different scheduling programs. The main program and the quantum chemistry
tasks are scheduled separately. Our local cluster provides for the
main program a separate queue (the serial queue), considering that
it requires only very small resources. To get the main program call
to run in the serial queue, request this queue in the batch script
({}``\#\$ -q serial{*}'') and do not request any CPUs (remove the
{}``\#\$ -pe'' line in the batch script). The last part is required
due to the specialty of the serial queue, which prevents that the
usual tasks might end there. The geometry single point calculations
are the ones who take most of the computational capacity. So if they
are scheduled independently, the usage of the CPUs should be the best.

The calls to the quantum chemistry program are submitted as independent
jobs into the queue. Thus it is always possible to start them all
at once. Only those which find a place on vacant processors will start
immediately. To have these tasks be recognized as different tasks
the call to the quantum chemistry program has to be changed. this
has to be done differently to the programs.
...
for the VASP command. The part {}``\textit{-now no'' }makes sure
that the scheduled task might wait in the queue. Without it it would
either start immediately or, if not enough CPUs are available, return
with an error message. The remaining part of the changed command should
be the same as was used in the serial context. 

\bigskip{}


The same can also be done for ParaGauss: set the cmdline not as described
in \ref{sub:The-ParaGauss-Calculator} but to:
...
The first part of the new command is the same as for VASP, note only
the difference in how it is provided to the program and the different
program itself. Other programs might be adapted similar.

\textbf{With Path Searching Programs}

For the path searching routines, like NEB, or the different string
methods, the cpu\_architecture has to be set to the wanted number
of jobs, which should run parallel. If all should start at the same
time cpu\_architecture should be set to the number of beads or larger.
If the number of jobs running at the same time should be limited the
cpu\_architecture should hold this value. The value can be provided
the same way as for the serial call, see \ref{sec:Serial-Calculations}.
...
If the cpu\_architecture was set in the parameter file (params.py)
the call to the path searcher should be adapted this way (for the
example ci-string):

\textit{paratools ci-string \nobreakdash-\nobreakdash-calculator
<CALC> \nobreakdash-\nobreakdash-paramfile params.py GEOM1 GEOM2}
...
\subsection{Running in Parallel with dplace (Hlrb2)}

One gets all the processes at once and then distribute the single
...
Here it is immense important, that the path searcher and frequency
tools get exactly the number of calculations which can/should run
in parallel. Otherwise they would leave processors idle or run several
jobs on the same ones (which will slow them down immensely). As already
described above the number of calculations, which should run in parallel,
are given to path searcher via the cpu\_architecture and to frequency
...
for path searcher) divided by the number of jobs running in parallel
is an integer. If you calculate for path searcher routines do not
consider the two minima, they will be only calculated once. Consider
that searching string only runs three geometry calculations on the
same time (only at the first call, where it also calculates the minima,
it runs 4).


\subsection{\textcolor{red}{Parallel Calculations on SuperMUC}}
...
\section{Approximations for the Transition State\label{sec:Approximations-for-the}}


\subsection{Path Used as Basis for the Transition State Approximation}

The different transition state methods available (excepts highest
bead) use a spline for the path. This spline is not completely consistent
with the one minimized by the string calculation. In the string calculation
the bead distances are obtained as the length along the previous build
path connecting the bead to its neighbor, while the path for the first
iteration was supposed to be equally spaced. The transition state
estimates are obtained with a path, where the distances are given
as the direct geometry distance to the neighbor bead, thus as if the
path would be linearly interpolated between the beads.
...
To get the path belonging to the state vector ($s\rightarrow x$):
\begin{itemize}
\item The default (old) path is build with the following algorithm (called
{}``with coordinate distance of beads'' in ts-and-mods):
...
\item spline approximation if more than 3 beads, else linear or quadratic
interpolation (in the last interpolation method the third bead is
always at $s=0.5$)
...
A spline for coordinates and a cubic spline for energy based on energy/gradients
on beads on either side of the expected transition state is used.
Program finds the maximum of the cubic spline and gives the corresponding
value for the coordinates back.
...
This time the energy inside these beads is described by a cubic function,
...
\textbf{Name} in ts-and-mods: {}``first to last bead''

The modevector is the (normed) vector connecting the two minima (first
and last bead)
...
\textbf{Name} in ts-and-mods: {}``directinternal''
...
\textbf{Name} in ts-and-mods: {}``frompath''
...
\subsection{Change in the routines}

All routines can now be accessed via the general command {}``paratools''
making it obsolete to have tools directory with many separate files
in PATH set.
...
more convenient. The direct usage of the tools from subdirectory is
...
Some of the parameter names in path searcher have been changed.

Path searcher allows ASE readable geometry files with arbitrary names,
if the format is explicitly given.
...
Several routines, which required explicitly a path.pickle file as
input, can now get information from files in user readable formats.


\section*{\textcolor{red}{Versions}}

This manual refers to the second completely released version of the
toolbox. Information to the most recent version may be found in the
...
\bibitem{wiki}http://intranet.theochem.tu-muenchen.de/wiki/index.php/ParaTools
...
by a Constrained Simplex Optimization Procedure, Klaus Müller and
