...
The difference here is whether they store them directly as a picture
(plot) or give them as a pop-up window (show). Table is connected
with them but will give the requested values in a table. The xyz tools
will give a string of xyz files, extracted from the current pickle
file. 
...
\subsection{The Tool for Extracting xyz Files From Pickle Files}


\subsection{Showing Arbitray Internal Coordinates Along a Path}


\subsubsection{The pat input and its variants}
...
exchangable but the default is a variant whichgenerates a parallel
...
if called in a batch job - a one to one usage of this strategy 
...
toolbox. Infromation to the most recent version may be found in the
