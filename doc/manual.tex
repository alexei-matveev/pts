...
and direct in a small python program. Some of the most important ones
are collected in the central command. These command is situated in
\textit{\$PYTHONPATH/pts/bin/} and is called paratools. The usage
is the following:
...
There are some other tools situated in \$PYTHONPATH/pts/tools. Most
of them are working on the output of a calculation to search a path
like on a string or neb calculation. But there are also some more
working independent of it. They are in separate files. Calling

\textit{<programfile> --help}

should always give a description of usage and options for each of
the tools. Here one can find:

\textit{tsestandmods.py <path.picke file>} 
...
}is ready to start. Please be aware that some of the tools are situated
in the /bin subdirectory, while others are in the /tools subdirectory.
\begin{itemize}
\item This setting should be setting three environment variables, thus for
a batch shell:


\textit{export PYTHONPATH=/users/alexei/PYTHON:\$\{PYTHONPATH\}}

\textit{export PATH=/users/alexei/PYTHON/pts/bin/:users/alexei/PYTHON/pts/tools/:\$\{PATH\}}
...
\textit{setenv PATH /users/alexei/PYTHON/pts/bin/:users/alexei/PYTHON/pts/tools/:\$\{PATH\}}
...
already contain the cell in a way, the toolbox can understand. The
toolbox can also understand the selective dynamics of the VASP POSCARs,
at least as long as all the calculations with the toolbox are performed
in Cartesian coordinates. Thus in our example the surface will be
fixed all the time, when the geometry is changed.
...
would use. The tool makepath.py (chapter \ref{sub:Makepath.py}) calculates
...
The so gotten file can be used to check the geometries with for example
jmol. In our case this will give figure \ref{fig:Path-of-linear}.
...
be chosen additionally. With this tool there will have to be done
a lot of calculations with the quantum chemistry program. There are
several geometries (of all the beads) known the same time. Thus it
makes sense to consider a parallel calculations. How parallel calculations
are done and what is special about the serial ones is described in
part \ref{par:Parallel-Calculations}. 

\item This calculation may finished because it hit the maximum iteration,
rather than because it converged. The convergence criteria will be
too high in this context. Still one can try to extract the best of
the results. For the last path there are some geometry files present,
...
finding one even better fitting for our purpose: The tool findlimitpath.py
...
findlimitpath.py searches automatically for the iteration where special
...
\caption{\textbf{Example output of tool findlimitpath.py (chapter \ref{sub:Findlimitaof.py}).
...
\begin{itemize}
\item All informations of interest are stored int the path.pickle files.
For each iteration there is one called string.debug?.path.pickle.
So the interesting one is string.debug30.path.pickle. First there
is a chance to get the path in xyz-format, for direct inspection,
even if there had not been any files generated yet. This can be done
by the tool path2xyz.py (chapter \ref{sub:Path2xyz.py}).


\textit{path2xyz.py string.debug30.path.pickle -b }

The outcome can be redirected to a file and shown with jmol for example
(figure \ref{fig:Path-30}).

\item To extract now the transition state, the tool tsestandmods.py (chapter
\ref{sub:Tsestandmods.py}) comes in handy. The following command
extracts the highest bead approximation and for two different path
descriptions (part \ref{sec:Approximations-for-the}) the spline and
cubic variant. The spine and cubic variant with with given distance
by string should be the best one. But in many cases the highest bead
is just sufficient.


\textit{tsestandmods.py string.debug30.path.pickle 1 4}

The result is also a xyz format output, which could be further used.
(If called with option --m, tsestandmods.py will also produce some
modevector, the one called frompath is especially for the spline and
...
with the tool path2xyz.py.}\label{fig:Path-30}}
...
energy path search.}\label{fig:Transition-state-estimate.}}
...
\item To watch the development of the path in special chosen internal coordinate
(independent of the coordinate system used for the path), there is
the tool path2plot.py (chapter \ref{sub:path2plot.py}). 


\textit{path2plot.py --dp 2 8 7 10 --dis 2 5 string.debug0.path.pickle}
...
to. The tool shows the path and the beads as points on it.} \label{fig:path2plot C-Ptpl H-C}}
...
The atoms objects, as described in the ASE framework, are also the
way to store molecules and atoms in ParaTools. They contain all kind
of data, like atom numbers, coordinates, forces, energies, ...

The unit cell and the periodicity can also be found/set there.

Most of these data can be set or read all the time. But of course
...
One way of defining atoms is for example by:
...
way or another make use of this read function. Thus they can be accessed
with all formats valid for ASE.
...
done on them. To learn more about the way the calculators have to
...
as none of the standard calculators, will be explained more in detail
separately. The extended one adds also some DFT-D energies to the
VASP results. They cannot be found in the ASE documentations. 
...
Which would start a minimization on the min1 geometry till the forces
are below 0.01 or 100 optimization steps is exceeded.
...
The TASK is set to GeoOpt and the MAX\_GEO\_ITERATION to zero: 
...
The tool pathsearcher.py is the general interface to the string and
NEB methods.

Usage:

\textit{pathsearcher.py \nobreakdash-\nobreakdash-calculator <calculator
filename> GEOM1 GEOM2}
...
they will be described in chapter \ref{sub:coordinateSystems}. 
...
or \textquotedbl{}gx\textquotedbl{} substrings. 

The file may also be in a format directly usable by ParaTools, thus
for example a zmatrix with all the initial values set correctly. Also
an mixed coordinate system specified as ccsspec object in a small
script is possible. There one can select by hand, which part is in
internal and which one is in Cartesian, with the possibility of giving
the initial values for the internal part also indirect over Cartesian
coordinates for the corresponding atoms. 

\bigskip{}


Another method of getting an internal (or mixed coordinate system)
is by giving the geometries in Cartesian (or as ase-files) and specifying
...
\subsubsection{Zmatrix\label{sub:Zmatrix-pathseracher}}
...
The angles are in degree. If only arbitrary values are set for being
overwritten by the Cartesian input make sure that no distance is set
to 0. This would lead to a crash. The values can be listed, but they
do not need to have a value given. If the zmatrix is only given additional
to Cartesian starting geometries the starting values would be replaced
anyhow. The last part of initialization the zmatrix values need not
even be there in this case.

The chapter \ref{sub:coordinateSystems} gives some more information
on coordinates and zmatrices.


\subsubsection{Setting Variables}

There are some other parameters specified, which decide on how the
program will run. There is a list of default parameters.

\textit{pathsearcher.py \nobreakdash-\nobreakdash-defaults}

shows all of them. They can be changed in two different ways:
\begin{itemize}
...
\item Or by giving directly for parameter\_to\_change:


\textit{\nobreakdash-\nobreakdash-<parameter to change> <new value>}

This only works for parameters which take a string, a float or a integer
(always a single number, or a name) as value. In the list of parameters
given below ch tells if they could changed by giving the parameter
values directly.

\end{itemize}
If the same variable is set in both the paramfile and directly, the
value from the direct set is taken.
...
There exists: (Parameter (ch): short description directly)
\begin{itemize}
\item cos\_type (yes):	 what calculation is really wanted, like NEB, string,
growingstring or searchingstring 
\item opt\_type (yes):	what kind of optimizer is used for changing the geometries
of the path, as default the new multiopt is used for the string methods,
while for NEB it is reset to ase\_lbgfs
\item pmax (yes):	maximal number of CPUs per bead, with our workarounds
normally only indirect used
\item pmin (yes):	minimal number of CPUs per bead, with our workarounds
normally only indirect used 
\item cpu\_architecture (no):	 describes the computer architecture, which
should be used; with our workaround only indirect used
\item name (yes):	the name of the calculation, appears as basis of the names
for all the output; needn't be set: as a default it takes the cos\_type
as name
\item calculator (yes):	the quantum chemistry program to use, like Vasp
or ParaGauss; for direct use a separate calculator file, in which
the calculator os set, is demanded
\item placement (no):	executable function for placing processes on beads,
only used for advanced calculations
\item cell (no):	the cell in which the molecule is situated
\item pbc (no):	which cell directions have periodic boundary conditions 
\item mask (no): which of the given geometry variables are supposed to be
changed (True) and which should stay fix during the calculation (False),
should be a string containing for each of the variables the given
value. The default does not set this variable and then all variables
are optimized
\item beads\_count (yes): 	how many beads (with the two minima) should be
used; growingstring and searchingstring start with less, for them
it is the maximum bead\_count
\item ftol (yes): 	the force convergence criteria, calculation stops if
RMS(force) < ftol 
\item xtol (yes): 	the step convergence criteria, only used if force has
at least ftol {*} 10 
\item etol (yes): 	energy convergence criteria, not really used 
\item maxit (yes): 	if the convergence criteria are still not met at maxit
iterations, the calculation is stopped anyhow
\item maxstep (yes): 	 the maximum step a path can take
\item spring (yes): 	the spring constant, only needed for NEB
\item pre\_calc\_function (no):	function for precalculations, for calculator
Gaussian etc.
\item output\_path (yes): 	place where most of the output is stored. The
working directory is not filled up too much, as all output goes there.
Default is subdirectory {}``workplace''
\item output\_geo\_format (yes): ASE format, in which the output-geometries
of the last iteration are written; it is xyz as default, but can be
changed for example to gx or vasp (POSCAR)
\item output\_level (yes):	the amount of output is decided here, for more
...
\begin{itemize}
\item 0: minimal output; not recommended; only logfile, geometries of the
beads for the last iteration (named BEAD?) and the output needed for
the calculation to run.
\item 1: recommended output level (default); additional to output of 0 there
are the ResultDict.pickle (usable for rerunning or extending the calculation
without having to repeat the quantum chemical calculations) and a
path.pickle of the last path; the path.pickle may be used as input
for some other tools, it stores the \textquotedbl{}whole\textquotedbl{}
path at it is in a special format. 
\item 2: additional a path.pickle for every path is created; good if development
of path is wanted to be seen (with the additional tools) 
\item 3: some more output in every iteration, for debugging etc. 
\end{itemize}
\bigskip{}


The ASE atoms objects may contain more information than only the chemical
symbols and the geometries of the wanted object. For example if reading
in POSCARs there are additional information about the cell (pbc would
be also set automatically to true in all directions). This information
can also be read in by this tool. If they are available, they are
only used, if these variables still contain the default parameters
after setting all other parameter. Additionally ASE can hold some
constraints, which may be taken from a POSCAR or can be set directly.
Some of them can be also used to generate a mask. This is only done
...
The calculators can be set up the same as described in chapter \ref{sub:The-calculators}.
There is no need to connect the calculator with anything. But therefore
it is absolutely needed, that the name of the calculator is calculator,
even if in general cases, as in chapter \ref{sub:The-calculators},
where it is used directly it could have been given any valid unused
name.


\subsubsection{Fixed Coordinates\label{sub:Fixed-Coordinates}}

There is the possibility to fix variables with the mask option. Then
...
to expect it to come from the input file. In POSCARs for example one
may use {}``selective dynamics'', which fix some of the coordinates.
This informations can be interpreted from ASE as constraints, which
then can be interpreted from ParaTools to fix some coordinates. For
general usage with the constraints (via python code): ParaTools can
interpret the {}``FixScaled'' and the {}``FixAtoms'' constraints
of ASE.
...
or the calculation should be repeated, this results can be reused
(the QC- program mustn't be changed, as well as the geometries of
the two minima). To reuse this results the parameters old-results
is used:

\textit{\nobreakdash-\nobreakdash-old\_results <filename>}
...
As a default the pathsearcher.py codes will have as initial path an
linear interpolation in all the variables of the two minima. But this
path may be totally off. First in calculations with periodic boundary
conditions one has to ensure that none of the atoms is in different
cells in the two geometries. But this may not be sufficient. in this
cases there is the chance on giving some additional geometries, which
should also be on the initial path. This may be due to the fact, that
not all internal variables are unique. For dihedral angles and the
objects describing the rotation of the complete internal system have
a $2\pi$ periodicity. The pathsearcher.py will take as a default
the shortest possible path. But if the longer one (with the norm of
dihdral angles larger than $\pi$ between the minima) are wanted there
is need for giving some more informations on the inital path. Note
that the reduction on angles smaller than $\pi$ is only done between
succeeding geometries, thus the comlete path may go over bigger angles.
This reduction is only done at the beginning before the first path
is generated. During the optimization the affected angles may of course
grow over $\pi$. 
...
\textit{pathsearcher.py \nobreakdash-\nobreakdash-paramfile <filename>
minima1 bead2 bead3 bead4 minima2}
...
\textit{pathsearcher.py \nobreakdash-\nobreakdash-calculator default\_lj
left.xyz right.xyz}
...
\subsection{Different Kinds of Coordinate Systems\label{sub:coordinateSystems}}
...
There are three different kinds of valid coordinate systems: Cartesians,
internals and mixed ones, where mixed ones refer to a system where
parts are Cartesians and parts are internal. It really depends, which
is to be preferred. But in some cases (see figure \ref{fig:Some-internal-coordinates})
using other than Cartesian make the starting interpolation much smother,
giving better changes for a fast and correct convergence.
...
\subsubsection{Internal Coordinates\label{sub:Internal-Coordinates/zmatrix}}
...
(1 for n=2). For the complete description there is also the need for
some additional coordinates, related to the orientation of all the
atoms in space. This is done by a RotAndTrans object, which holds
3 variables for translation and 2 to 3 for rotation (2 if there are
only 2 atoms). The rotation of the object is described with quaternions. 

\bigskip{}


The connectivities of the atoms amongst each are described with a
zmatrix and thus are given by the user. A complete zmatrix (as already
mentioned in \ref{sub:Zmatrix-pathseracher}) may look something like:
...
The first part gives the connectivities, the second part the initial
values for them. There has to be a blank line between the connectivity
tree and the initialization of the variables. 

In the connectivities the first thing in a line is the atom symbol.
The numbers of the atoms are given by their line number, starting
with 1. A line may be already finished after the atom symbol. If not
up to three variables follow, which connect it to the atoms already
defined. All the numbers given in a line before a variable name together
with the number of the current atom are used to generate the internal
coordinate of that variable name. Thus the first variable corresponds
to a distance of the new atom to the one belonging to the first number.
The second variable is about the angle (new atom with the ones of
the first two numbers). The third and last one is about the dihedral. 

The zmatrix does not allow several variables to have the same name,
even if they have the same value all the time.


\subsubsection{Mixed Coordinates}

This allows to separate all given atoms in several groups, respectively
several internal groups and one Cartesian one. This is only a limitation
of the pathsearcher.py wrapper. If the coordinate system is build
directly, there may be also several Cartesian blocks in between, and
the internal coordinates may be already initialized and all kind of
query things can be there. But even with the limitations of the pathsearcher.py
the mixed coordinates allow some more choice:

Anyhow this allows to group some atoms in their own internal coordinate
system, which will then be connected by its own RotAndTrans object
with the rest of the system. In the pathsearcher.py case all the atoms
have to be given also with their Cartesian coordinates, while the
zmatrices are given separately. If the zmatrices are initialized,
this will be overwritten. The program now takes as many atoms as needed
for the first zmatrix and generates the values for the internal coordinates,
with the coordinates of the RotAndTrans object such, that the atoms
are situated in space as they had been before. Then it repeats this
for all the other zmatrices. If there are still some atoms left, they
will be forwarded to the mixed coordinate system as Cartesians. The
variables of this system will then be all the internal and rest Cartesian
...
With a simple vasp string calculation as example the output is explained.
The output described here is just the same for the other cases, only
that the number of beads for the growing\_string and the searchingstring
method will be less at the beginning and grow later on.


\subsubsection{The Logfile}

\noindent The main information can be found in the .log file, here
named string.log. There the information of the string after n gradient/energy
calculations is summaries. This is the section between the \nobreakdash-\nobreakdash-\nobreakdash-\nobreakdash-\nobreakdash-\nobreakdash-\nobreakdash-
and ====== separators. The first section therein is for the whole
string:
...
The Step Size in the first iteration is of course everywhere zero,
it should vanish again to the end. The Bead Angles are calculated
between the different beads. When starting with a linear interpolation
they are always 180, but later on they change a lot. If they become
too small, there may be a problem with 'kinks' in the path. The Raw
State Vector gives the value for all Coordinates changing during the
calculation. So if there are some atoms fixed (for example a surface)
they would not appear here.
...
The third section holds general information:
...
and how many beads there are (currently).


\subsubsection{Geometries}

The geometry files BEAD\_n and TSESTIMATE\_m contain the geometries
of the last iteration, hopefully of the converged path. The TSESTIMATE
files contain the geometry of approximated transition states (the
MODEVEC\_M should contain a modevector as needed for a dimer calculation
for it, it uses the {}``frompath'' approximation for it). Compare
chapter \ref{sub:Available-Methods-TS} for the different available
methods, here the Spline and Cubic method is invoked. The output format
is xyz format unless output\_geo\_format does not say anything else.
The output\_geo\_format variable can be set to any of the formats
valid for the write command of ASE. The geometries will than be in
this output format instead of xyz. The BEAD\_n is the nth bead on
the last path.
...
path in an toolbox readable format (not human readable through). Thus
it can not used directly, but the toolbox provides a lot of other
nice tools, which uses this as an input. So for example the other
transition state estimate methods can be extracted from it.


\subsubsection{Other Output }

If the output\_level is raised, more output appear: for output\_level
= 2 for each iteration a path.pickle file is generated, some of the
other tools can make use of that very nicely. For output\_level =
3 there will be the state\_vec files for each iteration (as mentioned
in the initial path chapter \ref{sub:Initial-Path}). There will be
also some CoS.log's (for each iteration) which contains the path in
an ASE readable format. For this level there will also appear some
archive in the logfile.


\section{Additional Tools for the Path Searching (on Output Files)}

The following tools expect a finished path searching with one of the
pathsearcher.py functions. They are working on some output of it.
Especially the path.pickle files are used here. Only the path2plot.py
tool is somehow different: it uses some other tools, from which only
the first needs the pathsearcher.py output, the others may also be
used with files from other sources. Still the preference of it is
the usage of all three tools together as summarized in the path2plot.py.


\subsection{Analysis of archive File}

This is done by the program arcproc.py, which is localed in the /tools/
subdirectory.

Examines an archive ({*}archive.pickle) from a calculation and performs
any of a number of analysis tasks:

Usage: \textquotedbl{} + sys.argv{[}0] + \textquotedbl{} {[}options]
file.pickle {[}ts-actual.xyz] 

Options:

\textquotedbl{} -t : find TS approximation from path 

-g : display evolution of path in 3D 
...
The tool pp2tserr.py tells how far is a given transition state-estimate
from the true transition state, if the later is known. This tool is
mainly good for debugging. 

Usage: 

\textit{pp2tserr.py {[}options] file.pickle {[}ts-actual.xyz] }
...
The tool tsestandmods.py takes a path.pickle file and estimates the
transition states from it. Gives back the transition states (and the
modevectors) for different transition state estimates. First argument
is the path.pickle file to read in (or \textit{\nobreakdash-\nobreakdash-help}
to get a help text). Compare figure \ref{fig:Sometimes-the-highest}.
The usage is something like:

\textit{tsestandmods.py file.path.pickle }
...
All the different transition states and their procedures in detail
can be found in \ref{sec:Approximations-for-the}. In short it may
be noted that the Spline and cubic is supposed to be the best one,
it is also used as output for the transition state estimates for the
pathsearcher.py in its last iteration. Often it is just enough to
take the highest bead.
...
\nobreakdash-\nobreakdash- . The file need not be there, than all
...
for the current approximation (note that it will not check anything
else) and then use it instead of a new calculation. Missing forces
will be calculated as before. The file should be look like energyandforce.dat
generated with the \nobreakdash-\nobreakdash-ff option.
\item \nobreakdash-\nobreakdash-c {[}<logfile>]: for comparing the strings
with the previous ones. This option is mainly for debugging. The path,
which is build by the transition estimation tool may differ from the
one used for minimization. This difference should be very small compared
to convergence arguments. This tool gives as output relevant data
from the approximated string(s). The \nobreakdash-\nobreakdash-p
option is set to false by default. If the next argument is a file
name (for a logfile) the program searches in it for data to compare
the results with. In this case it needs also as an additional argument
a number, saying which iteration to take from. 
\end{itemize}
Except for the \nobreakdash-\nobreakdash-ff option all output goes
by default to the standard output.


\subsection{Plotting Abitrary Internal Coordinates Along a Path\label{sub:path2plot.py}}

The tool path2plot.py shows a picture of some preselected internal
coordinates, extracted from path.pickle files (compare figure \ref{fig:path2plot}).
As input there have to be some path.pickle file(s) and at least two
internal coordinates. Usage of it:

\textit{path2plot.py --dis 1 2 some.path.pickle --dis 1 3 }

%
\begin{figure}[H]
\includegraphics[scale=0.8]{Pictures/Ethanoxiddev_ito30}

\caption{\textbf{This tool is designed to show selected internal coordinates
...
An internal coordinate is selected by settings \nobreakdash-\nobreakdash-<kind>
<n1> <n2> ... where the ni's are the atom numbers (starting with 1)
which should be used for getting the internal coordinate. How many
...
The tool path2plot is in principle a shortcut for three different
tools, performed after each other. As an intermediate step may be
wanted another time or an intermediate step may be provided from another
tool or program, here is also the description of the three tools used:


\subsubsection{Path2xyz.py\label{sub:Path2xyz.py}}

The tool path2xyz.py takes a path.pickle file reads it in and gives
a string of jmol type xyz-files to the standard output. They are equally
...
\textit{path2xyz.py some.path.pickle N}

or simply:

\textit{path2xyz.py some.path.pickle}
...
\textit{path2xyz.py some.path.pickle -b }


\subsubsection{Xyz2tabint.py}
...
\textit{xyz2tabint.py --id 2 dis 1 2 dis 1 3 path.xyz}
...
\subsubsection{Tab2plot.py}

The tool tab2plot.py takes a tabular of some data (for example internal
...
\textit{tab2plot.py <file with table of data>}\bigskip{}
...
the option \textit{\nobreakdash-\nobreakdash-\textquotedbl{}k1 n1\_1
.. k2 n2\_1 ..\textquotedbl{}} can set the values. Here k can be chosen
from the following three values. The number following in bracket says
how many number for columns in the table have to be given for this
option to use. 
\begin{itemize}
\item t (1) : just take the column given by n1 
\item d (2) : difference between n1 - n2 
\item s (0) : gives the difference in the symmetry works on the next two
functions given and uses them to calculate 0.5 (k2(k1)-k2(-k1)) when
given a (float) number after s the k1 function values are shifted
about this number.
\end{itemize}
...
<n1> <n2>\textquotedbl{}}, the axes announced by the ni will be set
to logarithmic scale. Here the ni can be a number or x, y.
...
Findlimitpath.py was designed to work on the output data of the {*}.log
...
\textit{stayed below after: {[}2, 26, 21, 26, 26, 22, 2] f}

\textit{or all beads 26 with 0 times fallen above again }
...
\section{Additional Tools for the Path Searching (Without Previous Pathsearcher
Calculation)}


\subsection{Generating Interpolated Geometries for a Path\label{sub:Makepath.py}}

Makepath.py is a script to generate a path and then give back some
geometries lying equidistant on this path. It can interpolate in internal,
mixed or Cartesian coordinates. The input files can be in any ASE
format. The tool was designed to view a path without having to start
the pathsearcher.py for it. Thus it may be checked if an initial path
interpolates right, before starting an expensive calculation with
a pathsearcher.py. As pathsearcher.py there is the possibility to
give only the two minima as starting geometries of to provide several
geometries, which should be on the path.

usage:

\textit{makepath.py GEOL GEOR}

or 

\textit{makepath.py \nobreakdash-\nobreakdash-num 12 GEOL GEOTS
GEOR}
...
As in the mixed coordinate approach there is no need for any inital
values for the zmatrix variables, also not for the internal coordinate
system. If a mixed coordinate system with several internal coordinate
blocks is wanted, \textit{\nobreakdash-\nobreakdash-zmat }has to
be called several times. Note that the program will use the atoms
from top for the internal coordinates and will leave the last ones
in Cartesians.
\end{itemize}
...
For them makepath.py as well as pathsearcher.py will take the shortest
path, thus changes in the mentioned coordinates will only be below
$\pi$. If the long way is wanted, there has to be provided some more
intermediate geometries, which show the wanted path. 


\subsection{Help for Reordering of Atoms}

The tool labeller.py displays a couple of ASE GUIs to facilitate the
ordering of atoms in pairs of molecules.

Usage:

\textit{labeller.py {[}-h|\nobreakdash-\nobreakdash-help] mol1.xyz
mol2.xyz}

After the first window appears, you must click close. This doesn't
actually close it but allows the second window to open. I know that
doesn't make sense, it must be to do with a bug/quirk in ASE.

When the user clicks on an atom with the MIDDLE button, the program
prints to stdout the indices of the atoms in the order that they are
clicked on, and marks the atoms that have already been clicked.

By clicking on atoms in molecule 1, then molecule 2, then molecule
1, etc. the text which is written to stdout can by used by tools/rotate.py
to re-order and align molecules given in Cartesian coordinates.

For help use \textit{\nobreakdash-\nobreakdash-help.}


\subsection{Help to Aligne two Molecules}

Rotate.py two molecules specified in Cartesian coordinates and:
\begin{enumerate}
\item re-orders the atoms as specified 
\item rotates/translates the second molecule so that the specified atoms
optimally aligned.
\end{enumerate}
After performing the minimization, it displays the molecules graphically
so that the user can visually inspect whether the desired alignment
has been achieved. Two files named the same as the inputs but with
the '.t' extension are written and these contain the rotated/translated
geometries with the new ordering.

Usage:

\textit{rotate.py {[}-h, \nobreakdash-\nobreakdash-help] <mol1.xyz>
<mol2.xyz> \textquotedbl{}<ixs>\textquotedbl{} \textquotedbl{}<atom
pairs list>\textquotedbl{}}

where: <mol(1|2).xyz> are the molecules in Cartesian coordinates.

<ixs> is the indices (in Python list format) of atoms to use to measure
the alignment, and must have that the length > 3. Note that the <ixs>
are used on the atoms AFTER they have been re-ordered according to
<atom pairs list>.

<atom pairs list> is the list of atom indices to allow for the case
that atoms may be ordered differently in mol(1|2).xyz. For an identical
ordering, use \textquotedbl{}0 0 1 1 2 2 3 3 4 4\textquotedbl{}. For
a reversed ordering use \textquotedbl{}0 3 1 2 2 1 3 0\textquotedbl{}.
...
\textit{python rotate.py mol1.xyz mol2.xyz \textquotedbl{}{[}0,1,2]\textquotedbl{}
\textquotedbl{}0 3 1 1 2 0 3 2\textquotedbl{}}

for help use \textit{\nobreakdash-\nobreakdash-help} 


\subsection{Generating Zmatrix-coordinates from Zmatrix Objects and Geometries}

In principle this functionality is included in the pathsearcher.py
program. Xyz2zmat.py takes special xyz files and a zmatrix, with invalid
initialized values and produces internal zmatrix objects with values
obtained from the xyz file geometry.

It reads in three files: reactants.xyz.t, products.xyz.t and temp.zmt.

Then it takes the coordinates from the xyz files and the zmatrix connections
of the tmp.zmat and creates two files reactants.zmt and products.zmt,
which contains the atoms in zmatrix coordinate form with the starting
values already set, so that the corresponding xyz description would
be in the xyz files. This zmatrix output functions could also used
as geometry input for pathsearcher.py, which then does not need the
xyz files at all.


\section{Parallel Numerical Frequency Calculation\label{sec:Parallel-Numerical-Frequency}}

ParaTools includes a routine to calculate the frequencies of a given
molecule structure. This will be done by first calculating a numerical
Hessian matrix (second derivatives of the energy), by calculating
the forces of the molecule with a displacement in every valid geometry
direction (in Cartesian). Then the Hessian matrix and a mass-matrix
will be put into an eigensolver of the scipy module. The specialty
of this function is, that the calculations of all the displaced geometries
can be performed in absolute parallel. The way this parallel performing
is done is by using a {}``map''- function. If the tool is not used
interactively via the central command, this map-function can be chosen
directly and changed easily, as all the geometries are given to a
\textquotedbl{}map\textquotedbl{}-function. The map function could
be the simple (and non-parallel ) map function of python or one of
the map functions described in the pts.paramap module. In the central
command case there are two map-functions preselected, which can be
...
There are other options possible right now:
\begin{itemize}
\item \textit{\nobreakdash-\nobreakdash-alsovec True }: would tell to
the function, that the eigenvectors of the system are also wanted.
The output is enlarged by them.
...
which could be read in by ASE (compare chapter \prettyref{sub:Fixed-Coordinates}).
...
two different parallel mapping functions, which can be chosen in this
interactively manner. One of them does not need the knowledge about
the computer structure as it starts all single point force calculations
at the same time. The second uses the scheduler strategy, which is
also used in the pathsearcher. This one needs knowing how many processors
are there. They should be given to the function as n. Then the n processors
will be distributed in a fashion, which should allow the fastest calculation
of all the wanted points. This will be described further, as it is
...
just given one after another. The same is possible with the first
commando \textit{forces} instead of \textit{energy}.

The tool will give back the requested variable in eV (energy) or eV/Angstrom
(forces).


\subsection{Visulazing an Linear Interpolation with Jmol}

Run

\textit{jmol.py POSCAR1 POSCAR2 ... POSCARn}

or

\textit{jmol.py 1.xyz 2.xyz ... n.xyz}
...
\part{Parallel Calculations of Several Geometries\label{par:Parallel-Calculations}}
...
\section{Parallel Calculations with Fixed Processor Number Per Job}
...
\section{Parallel Calculation with Flexible Processor Number Per Job}
...
\bibitem{key-6}http://intranet.theochem.tu-muenchen.de/wiki/index.php/Interfacing\_ASE\_and\_ParaGauss
