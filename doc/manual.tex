...
approximated ``reaction'' coordinate there is the distance of the
second C atom to the Pt plane (the C which goes up), the y-coordinate
...
geometry file includes ``POSCAR'' or ``CONTCAR'' it will try to
read them in as VASP format. In this case it assumes, that the comment
line in it will be filled with the atom numbers or a corresponding
...
The format ``xyz'' would enforce for example also POSCAR.1 to be
read as xyz-file, ``vasp'' will accept everything as POSCAR and
``gx'' will expect a gxfile. Check the ASE manual for a complete
...
the ``FixScaled'' and the ``FixAtoms'' constraints of ASE. ParaTools
only fixes coordinates this way if they are the only mask available
and if only Cartesian coordinates are used.
...
are shown in ``Raw State Vector'' of the logfile are given (in Angstroms
or radients). Another one, ``abscissa'', contains their positions
on the path for path parameterizing, thus the first value should there
always be 0 and the last one 1. As the beads needn't be equal spaced
in the path varialbe, not even (completely) for a plain spline interpolation
this information might be valuable for further observations of the
path. As these values do not exist for neb calculations where the
spacing is done by the spring forces, a neb calculation will not create
this file. The file ``energies'' contains as it says the energies
of the beads (in eV) and ``forces'' the corresponding first derivatives
(in eV/Angstrom) to Cartesian coordinates.
...
used here is the ``spline and cubic'' method, compare chapter \ref{sub:Available-Methods-TS}.
Only if it fails the highest bead method is used. The output is the
same as for the bead<i>. The internal coordinates (as in logfile given)
are printed to the corresponding ts\_internals<i>.
...
(consisting of letters have to given with ``''). Thus in such a
file one could find the line:
...
the full name of it as a string (thus given with `` before and after),
then of course the limit and a special option, telling the kind of
the limit. So for each limit to search for, there should be something
like:
...
: Instead of ``RMS Perp Forces'' the line of interrest will be \textit{<String
as new argument>}
...
and ended marked with ``, should be a list containing for each Cartesian
geometry coordinate either the value True (this coordinate should
be used) or False (this coordinate is fixed and thus can be left away).
If it is not given all coordinates are taken as True, except when
the geometry file (like a POSCAR) contains some constraints, which
could be understand by ASE (compare chapter \prettyref{sub:Fixed-Coordinates}).
...
It is also recommended ot expand the PATH in order to have direct
access to the tools: 
...
between our ASE and the origional ASE in these sense.

The source of our ASE are managed locally by darcs revision control
system. There is for example the repository used for the calulations
at /users/alexei/PYTHON/ase. But this is only accessible within our
local network. Another version, which should have in general the same
content can be also accessed from outside. To see its content have
a look at:

\textit{http://129.187.157.201/cgi-bin/darcsweb.cgi?r=ase;a=summary}

or (if the computer is known to the calling computer)

\textit{http://quad4/cgi-bin/darcsweb.cgi?r=ase;a=summary}

The sources can be found at \textit{http://129.187.157.201/darcs/ase/}
(\textit{http://quad4/darcs/ase/}). 

To get these sources execute

\textit{darcs get http://129.187.157.201/darcs/ase}

To update the repository obtained in this fashion execute

\textit{darcs pull http://129.187.157.201/darcs/ase}
...
But the code is also available to the ouside. To get a relatively
stable version of the code: 

\textit{darcs get http://129.187.157.201/darcs/pts.2}

Accordingly one can have a look at the repository via

\textit{http://129.187.157.201/cgi-bin/darcsweb.cgi?r=pts.2;a=summary}

Remember that in our local network the computer with the given IP-adress
is also know as quad4.

Older versions of pts code and a newer developer version might be
also around at \textit{http://129.187.157.201/darcs/.}

To use the code one also ha to adapt the PYTHONPATH variable accordingly
(compare \ref{sub:Getting-ASE}). 
...
Another way to give all required informations to vasp is with a VASP\_SCRIPT.
...
\item dimer, lanczos: local transition state searches
...
\item path2plot, path2tab, path2xyz: takes path.pickle file and extracts
internal/cartesian coordinates
\item xyz2tab, tab2plot: extracted intermediate steps from path2plot
...
when and how many of them might be done in parallel. 
...
\subsection{Running in Parallel on Our Local Cluster\label{sub:Parallel-local-cluster}}
...
Verison 1.0: This manual refers to the first completly released version
of the toolbox. Infromation to the most recent version may be found
in the intranet. A general overview is located in \cite{intranet}.
A tutorial, showing the usage of some of the main features is in \cite{tutorial}.
Some of the tools have their own side there, so it may worth to have
a try.

Version 2.0: Update to Version2. Informations are also available in
\cite{wiki}. There is also a new tutorial availabe in \cite{tutorial2}.


\section{New Features in Version 2.0}
...
\section{Change in the Routines in Version 2.0}
...
\bibitem{tutorial2}http://intranet.theochem.tu-muenchen.de/wiki/index.php/Tutorial\_on\_features\_of\_ParaTools\_Version\_2
...
E, Weiqing Ren and Eric Vanden-Eijnden, Phys. Rev. B 66, 052301 (2002)\end{thebibliography}
