...
def main(argv):
    ...
    The  path  file can  be  in
    * path.pickle  format
    * or  alternatively containing  direct  internal   geometry
    coordinates  with the requirement of the remaining input in
    separate user readable files.
    Required are in this case the files of the --symbols option. Additional
    the transformation from internal to Cartesian coordiantes needs to
    ...
    Gives  back  the  transition  states  (and  the  modevectors)  for
    different transition state estimates.

    One  can  choose  which  transtition  state estimates  are  to  be
    generated by giving the numbers  N1, N2, ... related to the wanted
    ones. As default all available transition state estimates, except
    the three points result is given.
    ...
    parser.add_option("--cartesian-modes", dest = "printwithmodes",
                       help = "Append also the Cartesian mode vectors",
                       action = "store_true", default = False )

    parser.add_option("--m", "--modes", dest = "print_direct_modes",
    ...
    parser.add_option("--a", "--abscissa", dest = "abcis",
